#!/usr/bin/env python3
"""
TheHitterBot.py — Telegram card checker (Shopify + Adyen).
Companion files: helpers.py, adyen_engine.py
...
MASS_WORKERS = int(os.environ.get('MASS_WORKERS', '5'))

HIT_LOG_GC_ID = -1003962330916
...
async def _call_local_api(shop_url: str, card: str, proxy_raw: str):
    c = await _get_client()
    try:
...
def _grant_access(uid: int, hours: int, via_key: str):
    now = int(time.time())
    expires = now + int(hours) * 3600
    _users_access[str(uid)] = {
        'granted_at': now,
        'expires_at': expires,
        'via':        via_key,
    }
...
def _build_gc_log(result: dict, checker_name: str) -> str:
    ...
    if status == 'Charged':
        ...
        header = "Live Card"
    elif '3ds' in low or '3d secure' in low or 'challenge' in low:
        header = "3DS Hit"
    else:
        header = "Hit"
...
async def single_check(event):
    ...
    try:
        ...
        elif _is_insuf(result.get('message', '')):
            await _post_gc_log(result, cname)
        elif _is_3ds(result.get('message', '')):
            await _post_gc_log(result, cname)
    except Exception as e:
...
async def txt_detected(event):
    ...
    if not fp:
        await event.reply(pe(f"❌ <b>File download failed.</b>"), parse_mode='html')
        return
...
async def mass_check_cmd(event):
    ...
    if not reply.file or not reply.file.name.endswith('.txt'):
        await event.reply(pe(f"❌ <b>Invalid File</b>"), parse_mode='html')
        return
    sites   = load_sites()
    proxies = get_proxies_for_user(uid) or load_proxies()
...
async def cb_admin_list_users(event):
    ...
    if not combined:
        ...
        return
    lines = "\n".join(f"  {i+1}. <code>{u}</code>" for i, u in enumerate(combined))
    await event.answer()
    await nav_edit(event.chat_id, event.message_id, pe(f"<b>📋 Access List  ({len(combined)})</b>\n<b>{SEP}</b>\n{lines}"), [[{"text": "↪️  Back", "callback_data": "admin_users"}]])
...
async def cb_admin_add_site_info(event):
    if not is_admin(event.sender_id):
        ...
        return
    await event.answer()
    await nav_edit(event.chat_id, event.message_id, pe(f"<b>✅ Add Site</b>\n<b>{SEP}</b>\nUsage: <code>/addsite URL</code>"), [[{"text": "↪️  Back", "callback_data": "admin_sites"}]])
