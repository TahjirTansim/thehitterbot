...
def _normalize_selfhost(data: dict, card: str, proxy_raw: str) -> dict:
    ...
    # Force retry on captcha (site+proxy combo is blocked — try different combo)
...
async def test_proxy_async(p: dict):
    """Test a parsed proxy dict — returns dict on success, None on fail."""
    try:
...
def _proxy_url_to_raw(p: dict) -> str:
    """Store proxy as 'user:pass@host:port' or 'host:port'."""
    ip = p.get('ip') or ''
    port = p.get('port') or ''
...
def _should_post_to_gc(result: dict) -> bool:
    """Only Charged (bypassed, paid) and Insufficient Funds go to GC."""
...
async def send_3ds_log(user_id, result):
    ...
    # NOTE: 3DS hits are NOT posted to GC — user-private only.
...
async def single_check(event):
    ...
    # Proxy gate — user MUST have their own proxy
...
async def hit_command(event):
    ...
    smsg = await event.reply(pe(
        f"⚡ <b>Stripe Check...</b>\n"
        f"<b>{SEP}</b>\n"
        f"🔗 <code>{link[:60]}{'...' if len(link)>60 else ''}</code>\n"
        f"🃏 <tg-spoiler><code>{card}</code></tg-spoiler>\n"
        f"<b>{SEP}</b>\n"
        f"⏳ Please wait..."
    ), parse_mode='html')
    ...
    try:
        ...
        await smsg.edit(pe(
            f"❌ <b>Stripe Check Failed</b>\n"
            f"<b>{SEP}</b>\n"
            f"⚠️ <code>{str(e)[:150]}</code>"
        ), parse_mode='html')
...
async def setproxy_command(event):
    ...
    if not content:
        ...
        return
    # split glued concatenated lines if helpers available
...
async def addsite_command(event):
    ...
    urls = [u for u in candidates if u.startswith('http') and 'myshopify.com' in u.lower()]
    skipped = len(candidates) - len(urls)

    if not urls:
        await event.reply(pe(
            f"❌ <b>No valid Shopify sites found</b>\n"
            f"<b>{SEP}</b>\n"
            f"Sites must contain <code>myshopify.com</code>"
        ), parse_mode='html')
        return
...
# ── /addproxies — reply to .txt, test, save ────────────────────────
...
async def txt_detected(event):
    ...
    if not is_premium(uid):
        await event.reply(pe(f"❌ <b>Access Denied</b>"), parse_mode='html')
        return
    # Proxy gate — required for mass checks
    if not get_user_proxies(uid):
        await event.reply(PROXY_REQUIRED_MSG, parse_mode='html')
        return
    sites   = get_user_sites(uid) or get_live_sites()
